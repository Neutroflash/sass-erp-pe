...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await withTenantRLS(prisma, auth.tenantId, (tx) => tx.product.findFirst({ where: { id: params.id, tenantId: auth.tenantId } }));
  if (!existing) {
    return NextResponse.json({ error: "Producto no encontrado" }, { status: 404 });
  }

  const product = await withTenantRLS(prisma, auth.tenantId, (tx) => tx.product.update({ where: { id: params.id }, data: parsed.data }));
