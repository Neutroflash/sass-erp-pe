...
export default async function ReclamosPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Reclamos</h1>
      ...
      {complaints.length === 0 ? (
        <p className="text-muted-foreground">Todavía no llegó ningún reclamo o queja.</p>
      ) : (
            ...
            <tbody>
              {complaints.map((c) => (
                <tr key={c.id} className="border-b border-border/50 hover:bg-accent">
                  ...
                  <td className="px-4 py-3 text-foreground/90">{TYPE_LABEL[c.type] ?? c.type}</td>
                  <td className="px-4 py-3 text-foreground/90">{c.consumerName}</td>
                  <td className="px-4 py-3 text-muted-foreground">{c.createdAt.toLocaleDateString("es-PE")}</td>
                  <td className="px-4 py-3">
